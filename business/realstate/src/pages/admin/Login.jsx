...
const Login = () => {
  ...
  const [email, setEmail] = useState('admin@godmoon.com');
  const [password, setPassword] = useState('password123');
  const [showPassword, setShowPassword] = useState(false);

  const handleLogin = (e) => {
    e.preventDefault();
    if (email === 'admin@godmoon.com' && password === 'password123') {
      // Set dummy authentication state
      localStorage.setItem('isAdminLoggedIn', 'true');
      navigate('/admin');
    } else {
      alert("Invalid credentials. Please use admin@godmoon.com and password123");
    }
  };
